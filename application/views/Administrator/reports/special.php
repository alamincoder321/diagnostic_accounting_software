...
<script>
    ...
    new Vue({
        ...
        computed: {
            ...
            // total amount
            closeBalance() {
                return parseFloat(this.totalSaleAmount - this.returnAmount).toFixed(2);
            }
        },
        ...
        methods: {
            ...
            getBankPaid() {
                axios.post('/get_sales', this.filter)
                    .then(res => {
                        let groupedBankPaids = _.groupBy(res.data.sales.filter(item => parseFloat(item.SaleMaster_bankPaid) > 0), 'bank_id');
                        this.bankPaids = Object.keys(groupedBankPaids).map(bankId => {
                            let bankGroup = groupedBankPaids[bankId];
                            return {
                                bank_id: bankId,
                                bank_name: bankGroup[0].bank_name,
                                account_number: bankGroup[0].account_number,
                                amount: bankGroup.reduce((acc, cur) => acc + parseFloat(cur.SaleMaster_bankPaid), 0)
                            };
                        });                        
                    })
            },
